...
<section class="card"><div class="section-title">Management access</div><div class="actions"><input id="mkey" type="password" autocomplete="current-password" placeholder="CPA management key" style="max-width:420px;min-height:36px;border:1px solid var(--line-2);border-radius:8px;padding:8px 10px"><button class="btn primary" id="save-mkey" type="button">Use key</button><button class="btn" id="clear-mkey" type="button">Clear</button><span class="muted">Stored in this browser session only.</span></div></section>
<div class="provider-tabs"><button class="provider-tab active" type="button" data-provider-tab="agy2api">Antigravity</button><button class="provider-tab" type="button" data-provider-tab="gpt2api">ChatGPT</button></div>
...
document.querySelectorAll('[data-provider-tab]').forEach(node=>{ node.onclick = ()=>setProvider(node.dataset.providerTab); });
...
function syncModeButton(){ const node = el('toggle-mode'); if(!node){ return; } const on = !!(state.direct && state.direct.direct_mode_enabled); const noAccounts = accounts.length === 0; node.textContent = on ? 'Turn direct mode off' : (noAccounts ? 'Add account first' : 'Turn direct mode on'); node.disabled = !on && noAccounts; }

...
.share-layout{display:flex;gap:14px;align-items:flex-start}
.share-layout .donut-wrap{flex:0 0 auto;padding-top:4px}
.share-layout .share-list{flex:1;min-width:0}
.usage-trend{grid-column:1/-1}
.donut{width:168px;height:168px}
.donut-total{font-size:22px;font-weight:800;fill:var(--ink)}
.donut-caption{font-size:10px;fill:var(--ink-3);text-transform:uppercase;letter-spacing:.06em}
.trend{width:100%%;height:auto;display:block}
.grid-line{stroke:var(--line);stroke-width:1}
.axis-label{fill:var(--ink-3);font-size:9px}
.axis-label.rate{fill:#c026d3}
.bar.in{fill:#6366f1}.bar.out{fill:#10b981}.bar.cache{fill:#f59e0b}
.rate-line{stroke:#d946ef;stroke-width:1.6;stroke-dasharray:4 3}
.rate-dot{fill:#fff;stroke:#d946ef;stroke-width:1.4}
.chart-legend{display:flex;gap:14px;flex-wrap:wrap;margin-top:8px;font-size:11px;color:var(--ink-2)}
.chart-legend .swatch{display:inline-block;width:10px;height:10px;border-radius:3px;margin-right:5px;vertical-align:-1px}
.swatch.in{background:#6366f1}.swatch.out{background:#10b981}.swatch.cache{background:#f59e0b}
.swatch.rate{background:repeating-linear-gradient(90deg,#d946ef 0 4px,transparent 4px 7px);height:3px;border-radius:0;margin-top:4px}
...
function modelsView(account){ const rows = (account.models || []).map(m=>'<tr><td><code>' + esc(m.upstream_id) + '</code>' + (m.unavailable ? ' <span class="pill warn">unavailable</span>' : '') + '</td><td><code>' + esc(m.alias || '') + '</code></td><td>' + (m.enabled ? '<span class="pill ok">enabled</span>' : '<span class="pill">disabled</span>') + '</td><td>' + (m.image ? '<span class="pill">image</span>' : '') + '</td><td>' + (m.thinking ? '<span class="pill">thinking</span>' : '') + '</td></tr>').join(''); return '<div class="actions"><button class="btn" data-action="scan" data-account="' + esc(account.account_id) + '">Fetch models</button><button class="btn" data-action="sync-provider-models" data-account="' + esc(account.account_id) + '">Sync from provider</button><button class="btn" data-action="scan-upsert" data-account="' + esc(account.account_id) + '">Scan &amp; write models</button><button class="btn" data-action="publish" data-account="' + esc(account.account_id) + '">Preview provider update</button></div><div class="chips" style="margin:0 0 12px"><label class="chip" title="name column keeps the bare model slug — strips the ' + esc(account.prefix || '') + '/ namespace"><input type="checkbox" id="opt-raw-names"' + (account.raw_names ? ' checked' : '') + '> Raw names</label><label class="chip" title="plugin never writes the alias column; you fill aliases by hand"><input type="checkbox" id="opt-manual-alias"' + (account.manual_alias ? ' checked' : '') + '> Manual aliases</label><label class="chip" title="one catalog entry per model — drops the alias column and the provider prefix clone"><input type="checkbox" id="opt-single-id"' + (account.single_id ? ' checked' : '') + '> Single ID</label></div><div class="table-wrap"><table class="table"><thead><tr><th>Upstream model</th><th>Client alias</th><th>Enabled</th><th>Image</th><th>Thinking</th></tr></thead><tbody>' + rows + '</tbody></table></div>'; }

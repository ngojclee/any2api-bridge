...
<div class="shell">
<aside class="sidebar">
<div class="brand"><h1>Any2Api Bridge</h1><div class="muted">Direct account console</div></div>
<div class="nav-group"><div class="nav-title">Antigravity / AGY2API</div><button class="nav-item" data-provider="agy2api" data-view="accounts">Accounts</button><div class="nav-sub"><button class="nav-item" data-provider="agy2api" data-view="models">Models</button><button class="nav-item" data-provider="agy2api" data-view="headers">Headers</button><button class="nav-item" data-provider="agy2api" data-view="routing">Routing</button></div></div>
<div class="nav-group"><div class="nav-title">ChatGPT / GPT2API</div><button class="nav-item" data-provider="gpt2api" data-view="accounts">Accounts</button><div class="nav-sub"><button class="nav-item" data-provider="gpt2api" data-view="models">Models</button><button class="nav-item" data-provider="gpt2api" data-view="headers">Headers</button><button class="nav-item" data-provider="gpt2api" data-view="routing">Routing</button></div></div>
...
<main class="content">
<div class="top"><div><h2 id="page-title">Direct accounts</h2><div class="muted">Operational console for direct CPA channels. Secrets stay write-only; this page only shows configured state.</div></div><span class="pill">%s</span></div>
<div id="notice" class="notice hidden"></div>
<section class="card">
<div class="grid">
<div class="metric"><span>Direct mode</span><strong id="direct-mode">-</strong></div>
...
<section class="card"><div class="section-title">JSON result</div><pre class="result" id="result">No management call yet.</pre></section>
</main>
</div>
<script>
const state = %s;
...
function render(){ document.querySelectorAll('.nav-item').forEach(node=>{ node.classList.toggle('active', node.dataset.provider === provider && node.dataset.view === view || node.dataset.provider === 'global' && node.dataset.view === view && provider === 'global'); }); document.querySelectorAll('.tab').forEach(node=>node.classList.toggle('active', node.dataset.view === view)); el('provider-pane').classList.toggle('hidden', provider === 'global'); el('usage-pane').classList.toggle('hidden', !(provider === 'global' && view === 'usage')); el('logs-pane').classList.toggle('hidden', !(provider === 'global' && view === 'logs')); el('settings-pane').classList.toggle('hidden', !(provider === 'global' && view === 'settings')); el('page-title').textContent = provider === 'global' ? view[0].toUpperCase() + view.slice(1) : (provider === 'agy2api' ? 'Antigravity / AGY2API' : 'ChatGPT / GPT2API') + ' - ' + view; renderSummary(); renderProvider(); renderLogs(); renderSettings(); }
